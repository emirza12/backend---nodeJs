...
router.get('/', (req, res) => {
    res.status(200).json(learningpackage_1.default);
});
/**
 * @openapi
 * /api/package/{id}:
...
router.get('/:id', (req, res) => {
    ...
    const learningPackage = learningpackage_1.default.find(lp => lp.id === id);
    if (learningPackage) {
...
router.post('/api/package', (req, res) => {
    ...
    const newId = learningpackage_1.default.length ? Math.max(...learningpackage_1.default.map(lp => lp.id)) + 1 : 1;
    ...
    learningpackage_1.default.push(newLearningPackage);
...
router.put('/api/package', (req, res) => {
